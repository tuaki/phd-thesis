...
The core and the integration point of all our tools is an abstract representation of multi-model data called \textit{schema category}~\citep{mm_cat_journal}. It is defined as a tuple $\mathbf{S} = (\mathcal{O}_\mathbf{S}, \mathcal{M}_\mathbf{S}, \circ_\mathbf{S})$. \textit{Objects} in $\mathcal{O}_\mathbf{S}$ correspond to the domains of the ER model's entity types, attributes, and relationship types. 
