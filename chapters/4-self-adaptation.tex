...
In its most basic form, the problem we are trying to solve can be described as follows: given a fixed conceptual schema and a workload of queries, what set of logical models minimizes the overall query latency? And this is just a starting point, as many constraints (e.g., fixed schema and workload) can be relaxed. To define the problem more precisely, we derived the following open questions:

\question{sa:search-space}{Search space}{In general, there is inifinitely many ways to map the same conceptual schema to different logical models. We should find a deterministic way to limit the search space to a manageable size while still covering the best alternatives. Since the number of meaningful combinations grows exponentially with the size of the conceptual schema, the alternatives must be generated systematically and efficiently.}
...
\question{sa:latency-estimator}{Latency estimator}{To evaluate a state, the system has to at least estimate the latency of the queries in the workload. A naive solution (executing the queries) is extremely expensive. A much more efficient approach is required to make the search process feasible. Specifically, this solution cannot require actual materialization of the candidate state, as that is, by itself, too expensive.}
...
\question{sa:search-strategy}{Search strategy}{Even once a manageable state space is defined, the number of candidate representations grows too quickly for exhaustive evaluation. The system has to explore the space systematically, favor promising regions, and balance short-term exploitation with longer-term exploration.}

\question{sa:local-maxima}{Local maxima}{Cross-model queries are generally more expensive than single-model queries. Therefore, the search process may get stuck in local maxima, where any small change to the current state would lead to an increase in latency. Only larger changes (e.g., moving a whole subgraph to another model) would lead to a significant improvement. The search strategy should be able to escape local maxima and find better states that are not reachable by small incremental changes.}

\question{sa:migration-cost}{Migration cost}{Migration to the final representation is usually very expensive, as it may involve large data transfers and downtime. The performance gain of the new representation must be significant enough to justify the migration cost. The system has to be able to estimate the cost of migrating to (and maintaining) a candidate state.}
    
\question{sa:adaptation-timing}{Adaptation timing}{After a better representation is identified and the cost of migration is estimated, it is still unclear when the adaptation should be executed. Immediate adaptation may react quickly to workload changes but incur excessive migration overhead, while delayed adaptation may miss important optimization opportunities. The system should therefore decide when the expected long-term benefit outweighs the large one-time migration cost.}
...
\question{sa:richer-state-space}{Richer state space}{Database applications offer a wide range of physical-design choices beyond the logical model, such as indexes, materialized views, partitioning schemes, replication strategies, knob tuning, etc. These choices are not covered by the current categorical representation, but they can have a significant impact on query performance.}

\todo{reference open questions (self-adaptation)}

\subsection{TODO}

\todo{Co zatím máme - \paperRef{dke_2026} pro optimalizaci indexů, \paperRef{edbt_2026} pro otestování MCTS. Následně popsat celý proces + podrobně nastínit future work (MODELS 2026):

- High-level pohled (jak to interaguje se zbytkem systému, že vlastně potřebujeme všechny předchozí části).

- Samotný optimizér:

    - Co chceme podporovat (mapování, embeddingy, ISA, ...)

    - Reprezentace stavu (přes dotazy vs. přes kindy + embeddingy)

    - MCTS (co, jak, kdo a proč)

    - Latency estimator (neuronka)
