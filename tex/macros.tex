...
% A chapter, which is not numbered, but is included in the table of contents.
\def\chapterWithToc#1{
\chapter*{#1}
\addcontentsline{toc}{chapter}{#1}
}

% A section, which is not numbered, but is included in the table of contents.
...
\@ifclassloaded{article}{
  % Article doesn't know `chapter`, so we can't use it here. All examples will share the same counter (this is what we want in articles).
  \newtheorem{exampleinner}{Example}
}{
  % Examples will be prefixed with the chapter number; the counter will be reset in each chapter.
...
% See the comment in \publication.
\DeclareBibliographyCategory{publication}
...
% The paper can be referenced separately from the paper chapter. This is important for the thesis report, where the paper chapters are not included, but the list of publications is.
\label{pub:#1}%
\hangindent=2em\hangafter=1\relax%
% This is kinda complicated stuff but the point is that \fullcite creates [xyz] reference in the bibliography even if it's not actually referenced (i.e., there is no [xyz] in the text, because we have the full citation).
% So we want to prevent that ... but that's not trivial.
\nocite{#1}%
\addtocategory{publication}{#1}%
\begingroup
\let\blx@citation\@gobbletwo
\let\blx@hook@citekey\@empty
\fullcite{#1}%
\endgroup
% Show the "Paper XY" reference at the end of the citation if the paper is included in the thesis.
\ifcsname r@#1@display\endcsname
  \penalty0\null\nobreak\hfill\mbox{(\textbf{\paperRef{#1}})}%
\fi
\par
\vspace{1em}
}
